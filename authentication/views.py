...
def register_page(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            message = render_to_string(
                "boer-admin/authentication/email_template/activation_email.html",
                {
                    "user": user,
            ...
            email = EmailMultiAlternatives(
                mail_subject, plain_message, "butrosgroot@gmail.com", [user.email]
            )
            ...
            messages.success(
                request,
                "Account created. Please check your email for activation instructions.",
            )
    ...
    return render(request, "boer-admin/authentication/register.html", context)


def confirmation_sent(request):
    return render(request, "boer-admin/authentication/confirmation_sent.html")
...
def resend_confirmation_email(request, email):
    if getattr(request, "limited", False):
        messages.error(
            request,
            "Too many confirmation resent email. Please wait a minute and try again.",
        )
    ...
    if not user.is_active:
        ...
        messages.success(
            request, f"Confirmation email has been resent to {user.email}."
        )
    else:
...
def login_page(request):
    if request.method == "POST":
        ...
        remember_me = request.POST.get(
            "remember", False
        )  # Assuming the checkbox name is 'remember_me'
        next_page = (
            request.POST.get("next") or "booking_index"
        )  # This will use 'index' if next_page is None or empty
...
def send_2fa_email(request, email, code):
    if getattr(request, "limited", False):
        messages.error(
            request, "Too many 2fa login attempts. Please wait a minute and try again."
        )
        return redirect("login")

    mail_subject = "Your 2FA Code"
    message = render_to_string(
        "boer-admin/authentication/email_template/2fa_email.html",
        {
            "code": code,
        },
    )
    ...
    email_obj = EmailMultiAlternatives(
        mail_subject, plain_message, "butrosgroot@gmail.com", [email]
    )
...
def enter_2fa_code(request):
    ...
    return render(request, "boer-admin/authentication/enter_2fa_code.html", context)
