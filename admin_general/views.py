    ...
    if request.method == "POST":
        ...
        bookings = (
            Booking.objects.prefetch_related(
                "customer", "customer__user", "CampingSpot"
            )
            .order_by("start_date")
            .all()
        )
        ...
        for booking in paginator.page(page_number).object_list:
            ...
            if booking.CampingSpot is not None:
                booking_dict["Campingspot"] = (
                    booking.CampingSpot.plekNummer
                    if booking.CampingSpot.plekNummer is not None
                    else None
                )
            else:
            ...
            if booking.CampingSpot is not None:
                ...
                total_days_price = calc_full_price(
                    booking.start_date, booking.end_date, booking.CampingSpot.plekType
                )
            else:
                booking_dict["Campingspot"] = None
                total_days_price = 0
        ...
        if sort_by in valid_sort_fields:
            ...
            if order == "asc":
                bookings = bookings.order_by(
                    F("customer__user__last_name").asc(nulls_last=True)
                )
            elif order == "desc":
                bookings = bookings.order_by(
                    F("customer__user__last_name").desc(nulls_last=True)
                )
        elif sort_by == "email":
            if order == "asc":
                bookings = bookings.order_by(
                    F("customer__user__email").asc(nulls_last=True)
                )
            elif order == "desc":
                bookings = bookings.order_by(
                    F("customer__user__email").desc(nulls_last=True)
                )
        elif sort_by == "phone_number":
            if order == "asc":
                bookings = bookings.order_by(
                    F("customer__phone_number").asc(nulls_last=True)
                )
            elif order == "desc":
                bookings = bookings.order_by(
                    F("customer__phone_number").desc(nulls_last=True)
                )
...
def create_modal(request):
    if request.method == "POST":
        try:
            ...
            return JsonResponse(
                {"error": "Invalid JSON data in modal_data"}, status=400
            )
        ...
        if booking_id is not None:
            ...
            return JsonResponse(
                {"error": "Missing booking_id in modal_data"}, status=400
            )
...
def save_modal(request):
    if request.method == "POST":
        try:
            ...
            return JsonResponse(
                {"error": f"Missing or invalid modal_data: {e}"}, status=400
            )
...
def ble_logger(request, ip_adress, hostname, BLE_rssi, BLE_adress, BLE_name, BLE_count):
    BtMACAdress.objects.create(
        ip_adress=ip_adress,
        hostname=hostname,
        BLE_rssi=BLE_rssi,
        BLE_adress=BLE_adress,
        BLE_name=BLE_name,
        BLE_count=BLE_count,
    )
...
def ble_state_display(request):
    ...
    context = {"BLE_adresses": BLE_adresses}
...
def usage_data(request):
    ...
    bt_mac_data = BtMACAdress.objects.values(
        "ip_adress", "hostname", "BLE_count"
    ).order_by("date")
    ...
    return render(
        request, "boer-admin/admin_general/usage_data.html", {"data": sorted_data}
    )
...
def get_ble_data(request, ip_adress):
    ...
    ble_data_3_hours = BtMACAdress.objects.filter(
        ip_adress=ip_adress, date__gte=six_hours_ago
    ).order_by("date")
    ble_data_24_hours = BtMACAdress.objects.filter(
        ip_adress=ip_adress, date__gte=twenty_four_hours_ago
    ).order_by("date")
    ...
    for data in data_3_hours:
        if prev_data_3 != data["BLE_count"]:
            ...
            avg_data_3 = round(
                sum(window_3) / len(window_3)
            )  # Calculate and round the average
            new_data_3 = {"x": data["date"], "y": avg_data_3}
    ...
    for data in data_24_hours:
        if prev_data_24 != data["BLE_count"]:
            ...
            new_data_24 = {"x": data["date"], "y": avg_data_24}
            new_data_24_hours.append(new_data_24)
            prev_data_24 = data["BLE_count"]

    return JsonResponse(
        {"data_3_hours": new_data_3_hours, "data_24_hours": new_data_24_hours}
    )
