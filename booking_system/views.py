    ...
    if request.method == "POST":
        try:
            ...
            for plek_type in all_plek_types:
                number_of_spots[plek_type.id] = Veldvulling.objects.get(
                    PlekType=plek_type, Veld=veld
                ).number

            ## Check the number of CampingSpots for each PlekType and create them if needed
            for plek_type in all_plek_types:
                number_of_spots = Veldvulling.objects.get(
                    PlekType=plek_type, Veld=veld
                ).number
                number_of_spots_created = CampingSpot.objects.filter(
                    plekType=plek_type
                ).count()

                if number_of_spots_created < number_of_spots:
                    for i in range(number_of_spots_created, number_of_spots):
                        CampingSpot.objects.create(
                            plekNummer=i, plekType=plek_type, veld=veld
                        )
                ...
                return JsonResponse(
                    {
                        "status": "error",
                        "type": "no_places",
                        "message": "There are no places available for the selected time period.",
                    }
                )
            ...
            booking_created = Booking.objects.create(
                ...
                start_date=start,
                end_date=end,
                CampingSpot=accomodation_object,
            )
            ...
            total_price = calc_full_price(
                booking.start_date, booking.end_date, plek_type_object
            )
...
def booking_index(request):
    ...
    for place in places:
        price = Price.objects.filter(
            startDateTime__lte=now, endDateTime__gte=now, PlekType=place
        ).first()
        if price:
            places_dict.append(
                {
...
def beschikbaarheidsCheck(request):
    ...
    try:
        ...
        for place in available_spots.distinct("plekType").values(
            "plekType",
            "plekType__name",
            "plekType__fixed_price",
            "plekType__daily_price",
        ):
            fixed_price = Price.objects.filter(
                startDateTime__lte=start,
                endDateTime__gte=end,
                PlekType_id=place["plekType"],
                name=place["plekType__fixed_price"],
            ).first()
            ...
            daily_price = Price.objects.filter(
                ...
                endDateTime__gte=end,
                PlekType_id=place["plekType"],
                name=place["plekType__daily_price"],
            ).first()
        ...
        return JsonResponse(
            {"status": "success", "available_places": available_places_dict}
        )
...
def calc_full_price(start_date, end_date, plek_type_object):
    ...
    fixed_price = Price.objects.filter(
        startDateTime__lte=start_date,
        endDateTime__gte=end_date,
        PlekType=plek_type_object,
        name=plek_type_object.fixed_price,
    ).first()
    daily_price = Price.objects.filter(
        startDateTime__lte=start_date,
        endDateTime__gte=end_date,
        PlekType=plek_type_object,
        name=plek_type_object.daily_price,
    ).first()
